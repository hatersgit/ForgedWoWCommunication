...
local function JournalScrollButtons(frame)
	for i, bu in ipairs(frame.buttons) do
		...
		if frame:GetParent() == _G.WardrobeCollectionFrame.SetsCollectionFrame then
			bu.Favorite:SetTexture([[Interface\AddOns\ForgedWoW\UI\Common\FavoritesIcon]])
...
local function SkinToyFrame()
	...
	S:HandleButton_ezCollections(_G.ToyBoxFilterButton)
...
local function SkinTransmogFrames()
	...
	local function SkinCollectionModel(Model)
		...
		for i=1, Model:GetNumRegions() do
			...
			if region:IsObjectType('Texture') then -- check for hover glow
				...
				if texture:lower() == ([[Interface\AddOns\ForgedWoW\UI\Transmogrify\Transmogrify]]):lower() or texture:lower() == ([[Interface\AddOns\ForgedWoW\UI\Transmogrify\TransmogSetsVendor]]):lower() then
	...
	for i = 1, #WardrobeTransmogFrame.SlotButtons do
		...
		slotButton.NoItemTexture:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\Textures]]);
		slotButton.HiddenVisualCover:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\Transmogrify]]);
		slotButton.StatusBorder:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\Transmogrify]]);
	...
	for _, button in ipairs(WardrobeOutfitSlotsSelectionFrame.Buttons) do
		...
		button.IconOverlayApplied:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\Transmogrify]]);
		button.IconOverlayPending:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\PurpleIconAlertAnts]]);
	end
