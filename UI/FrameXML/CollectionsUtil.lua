...
function CollectionWardrobeUtil.SetAppearanceTooltip(tooltip, sources, primarySourceID, selectedIndex, showUseError, inLegionArtifactCategory, subheaderString)
	...
	if info and info.RaceMask then
		...
		elseif info.RaceMask == hordeRaceMask then
			...
			for i, name in ipairs(ezCollections.RaceIDToName) do
				if bit.band(info.RaceMask, bit.lshift(1, i - 1)) ~= 0 and ezCollections.L["Race."..i] and i < 12 and i ~= 9 then
					races = races .. (races ~= "" and ", " or "") .. ezCollections.L["Race."..i];
					if string.upper(name) == string.upper(select(2, UnitRace("player"))) then
						match = true;
					end
				end
			end
	...
	if info and info.ClassMask then
		...
		for i, name in ipairs(ezCollections.ClassIDToName) do
			if bit.band(info.ClassMask, bit.lshift(1, i - 1)) ~= 0 and LOCALIZED_CLASS_NAMES_MALE[name] then
	...
	if ezCollections.Config.Wardrobe.ShowItemID and not C_TransmogCollection.IsAppearanceHiddenVisual(headerSourceID) then
