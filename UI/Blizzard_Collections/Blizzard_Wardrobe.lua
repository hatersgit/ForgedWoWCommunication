...
function WardrobeFrame_OnLoad(self)
	...
	SetPortraitToTexture(WardrobeFramePortrait, [[Interface\AddOns\ForgedWoW\UI\Icons\INV_Arcane_Orb]]);
...
function WardrobeTransmogFrame_OnEvent(self, event, ...)
	if ( event == "TRANSMOGRIFY_UPDATE" or event == "TRANSMOGRIFY_ITEM_UPDATE" ) then
		...
		if ( slotButton ) then
			...
			if ( hasUndo ) then
				PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_VoidStorage_Undo.wav]]);
			elseif ( not hasPending ) then
				if ( slotButton.hadUndo ) then
					PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_VoidStorage_Redo.wav]]);
...
function WardrobeTransmogFrame_OnShow(self)
	...
	if ezCollections.Config.Wardrobe.EtherealWindowSound then
		PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_EtherealWindow_Open.wav]]);
	else
...
function WardrobeTransmogFrame_OnHide(self)
	if ezCollections.Config.Wardrobe.EtherealWindowSound then
		PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_EtherealWindow_Close.wav]]);
	else
...
function WardrobeTransmogFrame_UpdateSlotButton(slotButton)
	...
	if ( hasChange and (hasUndo or isPendingCollected and not slotButton.invalidWeapon) and not slotButton.AnimFrame:IsShown() ) then
		...
		if slotFailReason then
			...
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\ForgedWoW\UI\RedIconAlertAnts]]);
			slotButton.failed = true;
		else
			slotButton.PendingFrame.Glow:SetVertexColor(1, 1, 1);
			slotButton.PendingFrame.Ants:SetTexture([[Interface\AddOns\ForgedWoW\UI\Transmogrify\PurpleIconAlertAnts]]);
		end
		if ( hasUndo ) then
...
function WardrobeTransmogFrame_OnTransmogApplied()
	PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_Transmogrify_Apply.wav]]);
...
function WardrobeTransmogButton_OnClick(self, button)
	...
	if ( button == "RightButton" ) then
		if ( hasPending or hasUndo ) then
			PlaySoundFile([[Interface\AddOns\ForgedWoW\Sounds\UI_Reforging_Restore.wav]]);
...
function WardrobeItemsCollectionMixin:FilterVisuals()
	...
	local showObtainable = ezCollections:GetCVarBool("transmogrifyShowObtainable");
	local showUnobtainable = ezCollections:GetCVarBool("transmogrifyShowUnobtainable");
	...
	for i = 1, #visualsList do
		if ( isAtTransmogrifier ) then
			...
			elseif not visualsList[i].isCollected and showClaimable and ezCollections:CanClaimSkin(visualsList[i].visualID) then
				tinsert(filteredVisualsList, visualsList[i]);
			elseif not visualsList[i].isCollected and showPurchasable and visualsList[i].isStoreSource then
				tinsert(filteredVisualsList, visualsList[i]);
...
function WardrobeItemsCollectionMixin:UpdateItems()
	...
	for i = 1, self.PAGE_SIZE do
		...
		if ( visualInfo ) then
			...
			model.SolidBackground:SetShown(true);
...
function WardrobeItemsModelMixin:OnEnter()
	...
	if ( self:GetParent().transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
		...
		if ezCollections.Config.Wardrobe.ShowItemID and not C_TransmogCollection.IsAppearanceHiddenVisual(self.visualInfo.sourceID) then
			...
			for _, slot in ipairs({ 1, 3, 15, 5, 19, 4, 9, 10, 6, 7, 8, 16, 17, 18 }) do
				...
				if slotState[slot] ~= nil and slotName then
					...
					for _, source in ipairs(slotSources) do
						...
						if not hasNotStoreOnlySources then
...
function WardrobeTransmogFrameSpecDropDown_Initialize()
	...
	info.checked = false;
	info.text = format([[|TInterface\AddOns\ForgedWoW\UI\Common\spacer:1:20:-1:-1|t |T%s:22:22:0:-1|t %s (%s)]], icon, name, GetSpecialization() == 1 and TALENT_SPEC_PRIMARY or TALENT_SPEC_SECONDARY);
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
end
