...
function GetPetTypeTexture(petType)
	if PET_TYPE_SUFFIX[petType] then
		return [[Interface\AddOns\ForgedWoW\UI\PetBattles\PetIcon-]]..PET_TYPE_SUFFIX[petType];
	else
		return [[Interface\AddOns\ForgedWoW\UI\PetBattles\PetIcon-NO_TYPE]];
	end
end
...
function PetJournal_UpdatePetList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedPets and showPets ) then
			...
			button.name:SetText("");
			button.icon:SetTexture([[Interface\AddOns\ForgedWoW\UI\PetBattles\MountJournalEmptyIcon]]);
			button.new:Hide();
			button.newGlow:Hide();
			button.index = nil;
			button.spellID = 0;
...
function PetJournal_UpdatePetDisplay()
	if ( PetJournal.selectedPetID ) then
		...
		PetJournal.TypeInfo.typeIcon:SetTexture([[Interface\AddOns\ForgedWoW\UI\PetBattles\PetIcon-]]..PET_TYPE_SUFFIX[petType]);
