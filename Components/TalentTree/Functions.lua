...
function UpdateTalentCurrentView()
    ...
    if CurrentTab then
        if CurrentTab.TalentType == CharacterPointType.FORGE_SKILL_TREE then
            InitializeViewFromGrid(TalentTreeWindow.GridForgeSkill,
                CurrentTab.Talents, CurrentTab.Id, 465);
        else
            InitializeViewFromGrid(TalentTreeWindow.GridTalent,
                CurrentTab.Talents
                , CurrentTab.Id, 392);
        end
    end
    ...
    if frame then
        ...
        if IsUnlocked(CurrentRank, talent.NumberOfRanks) then
            frame.Border:SetBackdrop({
                bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\border_unlocked"
            })
        else
            if CurrentRank ~= -1 then
                frame.TextureIcon:SetDesaturated(nil);
                frame.Border:SetBackdrop({
                    bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\border_active"
                })
                ...
                if CurrentRank < 1 then
                    ...
                    frame.Border:SetBackdrop({
                        bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\border_locked"
                    })
                else
                    frame.Border:SetBackdrop({
                        bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\border_active"
...
function DrawNode(startPosition, endPosition, parentFrame, parent, offSet, talent, CurrentRank, previousSpell)
    ...
    if not parentFrame.node[talent.SpellId] then
        ...
        parentFrame.node[talent.SpellId].animation.spin = parentFrame.node[talent.SpellId].animation:CreateAnimation("Rotation")
...
function SelectTab(tab)
    ...
    if tab.TalentType == CharacterPointType.RACIAL_TREE or
        tab.TalentType == CharacterPointType.TALENT_SKILL_TREE or
        tab.TalentType == CharacterPointType.PRESTIGE_TREE then
        InitializeGridForTalent();
        if tab.Talents then
            InitializeViewFromGrid(TalentTreeWindow.GridTalent, tab.Talents,
                tab.Id, 392);
        end
...
function InitializePreviewSpecialization(tabId)
    TalentTreeWindow.SpecializationPreview = CreateFrame("Frame",
        TalentTreeWindow.SpecializationPreview, TalentTreeWindow)
...
function InitializeTalentLeft()
    ...
    TalentTreeWindow.TabsLeft = CreateFrame("Frame", TalentTreeWindow.TabsLeft,
        TalentTreeWindow);
    ...
    for index, tab in ipairs(TalentTree.FORGE_TABS) do
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureNormal = TalentTreeWindow.TabsLeft
            .Spec
            [tab.Id]:CreateTexture("$parentNormalTexture", "ARTWORK");
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureHighligted = TalentTreeWindow.TabsLeft
            .Spec
            [tab.Id]:CreateTexture("SetHighlightTexture", "ARTWORK");
        TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureHighligted:SetAllPoints();
        TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureHighligted:SetTexture(CONSTANTS.UI.SPECIALIZATION_BUTTON_BG_HOVER_OR_PUSHED)
        TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureHighligted:SetTexCoord(0, 0.625, 0.265625, 0);

        TalentTreeWindow.TabsLeft.Spec[tab.Id].LockedTexture = TalentTreeWindow.TabsLeft.Spec
            [tab.Id]:CreateTexture("$parentNormalTexture", "ARTWORK");
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id]:SetHighlightTexture(TalentTreeWindow.TabsLeft.Spec[tab.Id].TextureHighligted)
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id].Icon = CreateFrame("Frame",
            TalentTreeWindow.TabsLeft.Spec[tab.Id].Icon,
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id].Icon.Texture = TalentTreeWindow.TabsLeft.Spec[tab.Id].Icon:
            CreateTexture("$parentNormalTexture", "ARTWORK");
        ...
        TalentTreeWindow.TabsLeft.Spec[tab.Id].Circle = CreateFrame("Button",
            TalentTreeWindow.TabsLeft.Spec[tab.Id].Circle,
            TalentTreeWindow.TabsLeft.Spec[tab.Id]);
        ...
        if tab.TalentType ~= CharacterPointType.SKILL_PAGE then
            TalentTreeWindow.TabsLeft.Spec[tab.Id].Points = CreateFrame("Button",
                TalentTreeWindow.TabsLeft.Spec[tab.Id].Points,
                TalentTreeWindow.TabsLeft.Spec[tab.Id].Circle);
            ...
            TalentTreeWindow.TabsLeft.Spec[tab.Id].Points.Text = TalentTreeWindow.TabsLeft.Spec[tab.Id].Points:
                CreateFontString();
...
function InitializeGridForTalent()
    ...
    for i = 0, 21 do
        ...
        for j = 0, 21 do
            ...
            TalentTreeWindow.GridTalent.Talents[i][j] = CreateFrame("Button",
            ...
            TalentTreeWindow.GridTalent.Talents[i][j].Ranks:SetBackdrop({
                bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\rank_placeholder"
            })
            TalentTreeWindow.GridTalent.Talents[i][j].RankText = TalentTreeWindow.GridTalent.Talents[i][j].Ranks
                :CreateFontString()
            TalentTreeWindow.GridTalent.Talents[i][j].RankText:SetFont("Fonts\\FRIZQT__.TTF"
                , 9, "OUTLINE")
...
function InitializePreReqAndDrawNodes(spells, spellNode, children, parent, offset, CurrentRank)
    for _, pr in pairs(spellNode.Prereqs) do
        ...
        local startPosition = GetPositionXY(children[tonumber(previousSpell.ColumnIndex)][
            tonumber(previousSpell.RowIndex)]);
        ...
        DrawNode(endPosition, startPosition,
            children[tonumber(previousSpell.ColumnIndex)][tonumber(previousSpell.RowIndex)], parent, offset,
            spellNode, CurrentRank, previousSpell);
    end
end
...
function CreateTooltip(spell, SpellId, NextSpellId, parent, CurrentRank)
    FirstRankToolTip:SetOwner(parent,
        "ANCHOR_RIGHT");
    SecondRankToolTip:SetOwner(parent,
        "ANCHOR_RIGHT");
    ...
    if not NextSpellId and tonumber(spell.RankCost) > 0 and (CurrentRank < tonumber(spell.NumberOfRanks)) then
        FirstRankToolTip:SetSize(FirstRankToolTip:GetWidth(),
            FirstRankToolTip:GetHeight() + 28)
    end
    if NextSpellId then
        FirstRankToolTip:AddLine(" ");
        ...
        FirstRankToolTip:SetSize(FirstRankToolTip:GetWidth(), FirstRankToolTip:GetHeight() + SecondRankToolTip:GetHeight() + 40)
    end
end
...
function InitializeMiddleSpell()
    if TalentTreeWindow.GridForgeSkill.Talents
        and TalentTreeWindow.GridForgeSkill.Talents[11][5].Ranks then
...
function InitializeGridForForgeSkills()
    ...
    TalentTreeWindow.GridForgeSkill = CreateFrame("Frame", TalentTreeWindow.GridForgeSkill,
        TalentTreeWindow.Container);
    ...
    for i = 1, 20 do
        ...
        for j = 1, 9 do
            TalentTreeWindow.GridForgeSkill.Talents[i][j] = CreateFrame("Button",
                ...
                TalentTreeWindow.GridForgeSkill);
            ...
            TalentTreeWindow.GridForgeSkill.Talents[i][j].TextureIcon = TalentTreeWindow.GridForgeSkill.Talents[i]
                [j
                ]:CreateTexture();
            TalentTreeWindow.GridForgeSkill.Talents[i][j].TextureIcon:SetAllPoints()

            TalentTreeWindow.GridForgeSkill.Talents[i][j].Border = CreateFrame("Frame",
            ...
            TalentTreeWindow.GridForgeSkill.Talents[i][j].Exclusivity = CreateFrame("Frame",
                TalentTreeWindow.GridForgeSkill.Talents[i][j].Exclusivity,
                TalentTreeWindow.GridForgeSkill.Talents[i][j])
            ...
            TalentTreeWindow.GridForgeSkill.Talents[i][j].Ranks = CreateFrame("Frame",
                TalentTreeWindow.GridForgeSkill.Talents[i][j].Ranks, TalentTreeWindow.GridForgeSkill.Talents[i][j]);
            ...
            TalentTreeWindow.GridForgeSkill.Talents[i][j].RankText = TalentTreeWindow.GridForgeSkill.Talents[i][j]
                .Ranks
                :CreateFontString()
            TalentTreeWindow.GridForgeSkill.Talents[i][j].RankText:SetFont("Fonts\\FRIZQT__.TTF"
                , 9, "OUTLINE")
...
function ShowForgeSkill(tab)
    InitializeViewFromGrid(TalentTreeWindow.GridForgeSkill, tab.Talents,
        tab.Id, 465);
    ...
    TalentTreeWindow.Container:SetBackdrop({
        bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\SkillForge",
    });
    TalentTree.FORGE_SELECTED_TAB = tab;
end
...
function ShowSpellsToForge(spells)
    ...
    for index, spell in pairs(spells) do
        ...
        TalentTreeWindow.SpellBook.Spells[index].Icon = CreateFrame("Button",
            TalentTreeWindow.SpellBook.Spells[index].Icon,
        ...
        TalentTreeWindow.SpellBook.Spells[index].Points = CreateFrame("Button",
            TalentTreeWindow.SpellBook.Spells[index].Points,
            TalentTreeWindow.SpellBook.Spells[index].Icon);
        ...
        TalentTreeWindow.SpellBook.Spells[index].Points.Text = TalentTreeWindow.SpellBook.Spells[index].Points:
            CreateFontString();
        ...
        TalentTreeWindow.SpellBook.Spells[index].SpellName = TalentTreeWindow.SpellBook.Spells[index].Icon
            :
            CreateFontString()
        ...
        TalentTreeWindow.SpellBook.Spells[index].Texture = CreateFrame("Frame",
            TalentTreeWindow.SpellBook.Spells[index].Texture,
            TalentTreeWindow.SpellBook);
        ...
        TalentTreeWindow.SpellBook.Spells[index].Spell = CreateFrame("Button",
            TalentTreeWindow.SpellBook.Spells[index].Spell,
            TalentTreeWindow.SpellBook);
        ...
        TalentTreeWindow.SpellBook.Spells[index].Hover = CreateFrame("Frame",
            TalentTreeWindow.SpellBook.Spells[index].Hover,
            TalentTreeWindow.SpellBook);
        ...
        TalentTreeWindow.SpellBook.Spells[index].Hover:SetBackdrop({
            bgFile = "Interface\\AddOns\\ForgedWoW\\UI\\over-btn-forge",
        });
...
function InitializeTabForSpellsToForge(SkillToForges)
    ...
    TalentTreeWindow.SpellBook.NextArrow = CreateFrame("Button", TalentTreeWindow.SpellBook.NextArrow,
        TalentTreeWindow.SpellBook);
    TalentTreeWindow.SpellBook.NextArrow:SetNormalTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-NextPage-Up")
    TalentTreeWindow.SpellBook.NextArrow:SetPushedTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-NextPage-Down")
    TalentTreeWindow.SpellBook.NextArrow:SetDisabledTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-NextPage-Disabled")
    ...
    TalentTreeWindow.SpellBook.PreviousArrow:SetNormalTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-PrevPage-Up")
    TalentTreeWindow.SpellBook.PreviousArrow:SetPushedTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-PrevPage-Down")
    TalentTreeWindow.SpellBook.PreviousArrow:SetDisabledTexture("Interface\\AddOns\\ForgedWoW\\UI\\Buttons\\UI-SpellbookIcon-PrevPage-Disabled")
