...
function GetTalentTreeLayout(msg)
    ...
    for i, tab in ipairs(listOfObjects) do
        if tab.TalentType == CharacterPointType.TALENT_SKILL_TREE
            or tab.TalentType == CharacterPointType.RACIAL_TREE
            or tab.TalentType == CharacterPointType.PRESTIGE_TREE
            or tab.TalentType == CharacterPointType.SKILL_PAGE then
...
SubscribeToForgeTopic(ForgeTopic.LEARN_TALENT_ERROR,
    function(msg)
        print(msg);
    end)
SubscribeToForgeTopic(ForgeTopic.GET_TALENTS,
    function(msg)
